 ...
 *
 */
public class TodaysInstructionListTest {

	@Test
	public void testTodaysInsructionList() {
		...
		Calendar settlementDate1 = new GregorianCalendar(2017, 3, 15);
		...
		instructionBuilder.createInstruction(entity, Indicator.BUY, 0.25, Currency.SGP, incomingDate, settlementDate,
				200, 67.89);
		instructionBuilder.createInstruction(entity1, Indicator.BUY, 0.25, Currency.SGP, incomingDate1, settlementDate1,
				200, 67.89);
		...
		Iterator<Instruction> itr = list.fetchInstructionList().iterator();
		while (itr.hasNext()) {
			entityName.add(itr.next().getEntity().getName());
		}

		List<String> entityNameTest = new ArrayList<String>();
		List<String> entityNameTest1 = new ArrayList<String>();
		entityNameTest.add("foo");
		//entityNameTest.add("testSGP");
		entityNameTest1.add("foo");
		entityNameTest1.add("testSGP");
		entityNameTest1.add("testAED");
		boolean pass = entityNameTest.containsAll(entityName) && entityName.containsAll(entityNameTest);
		assertEquals(true, pass);
		boolean fail = entityNameTest1.containsAll(entityName) && entityName.containsAll(entityNameTest1);
		assertNotEquals(true, fail);
	}
